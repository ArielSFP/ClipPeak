...
import os
pretrained_model_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "models", "pretrain_TalkSet.model"))
save_path = "save/"
...
def evaluate_network(s, files):
	...
	for file in tqdm.tqdm(files, total = len(files)):
		# Use os.path.basename for cross-platform compatibility (Windows uses backslashes)
...
def main(
	...
	# Windows-compatible: use double quotes and absolute path
	video_path_abs = os.path.abspath(video_path)
	command = ('ffprobe -v error -select_streams v:0 -count_packets -show_entries stream=nb_read_packets -of csv=p=0 "%s"' % video_path_abs)
	...
	# get video length
	command = ('ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "%s"' % video_path_abs)
	...
	# get fps
	command = ('ffprobe -v error -select_streams v:0 -show_entries stream=r_frame_rate -of default=noprint_wrappers=1:nokey=1 "%s"' % video_path_abs)
	...
	command = ('ffmpeg -y -i "%s" -ss %s -t %s -qscale:a 0 -ac 1 -vn -threads %d -ar 16000 "%s" -loglevel panic' % \
		(video_path_abs, start_seconds, duration, data_loader_thread, os.path.abspath(audioFilePath)))
			...
			try:
				...
				try:
					...
					command = ('ffmpeg -y -i "%s" -vf fps=25 -ss %s -t %s -threads %d -f image2 "%s" -loglevel panic' % \
						(video_path_abs, start, duration, data_loader_thread, os.path.join(pyframesPath, '%06d.jpg')))
					subprocess.call(command, shell=True, stdout=None)
